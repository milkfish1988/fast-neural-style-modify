...
--     shotcut:add(nn.SpatialConvolution(dim_in, dim_out, 3, 3, 2, 2, 1, 1))
--     shotcut:add(nn.InstanceNormalization(dim_out))
--     concat:add(shotcut)
--   else
...
local function build_conv_block(dim_in, dim_out, padding_type, use_instance_norm, stride)
  ...
  conv_block:add(nn.ReLU(true))
  -- if stride == 2 then
    conv_block:add(nn.SpatialDilatedConvolution(dim_in, dim_out, 3, 3, 1, 1, 2, 2, 2, 2))
    conv_block:add(nn.InstanceNormalization(dim_out))
    conv_block:add(nn.ReLU(true))
  -- end
  conv_block:add(nn.SpatialConvolution(dim_out, dim_out, 3, 3, 1, 1, 1, 1))
...
local function build_res_block(dim_in, dim_out, padding_type, use_instance_norm, stride)
  ...
  if stride == 2 then
    shotcut = nn.Sequential()
    shotcut:add(nn.SpatialDilatedConvolution(dim_in, dim_out, 3, 3, 1, 1, 2, 2, 2, 2))
    shotcut:add(nn.InstanceNormalization(dim_out))
    concat:add(shotcut)
  else
    concat:add(nn.Identity())
...
--   seq3:add(bch3)
--   seq3:add(nn.SpatialConvolution(256, 64, 3, 3, 1, 1, 1, 1))
--   seq3:add(nn.InstanceNormalization(64))
--   seq3:add(nn.ReLU(true))  
--   seq3:add(nn.SpatialFullConvolution(64, 64, 3, 3, 2, 2, 1, 1, 1, 1))
...
--   seq2:add(bch2)
--   seq2:add(nn.SpatialConvolution(128, 32, 3, 3, 1, 1, 1, 1))
--   seq2:add(nn.InstanceNormalization(32))
--   seq2:add(nn.ReLU(true))  
--   seq2:add(nn.SpatialFullConvolution(32, 32, 3, 3, 2, 2, 1, 1, 1, 1))
...
--   seq5 = nn.Sequential()
--   seq5:add(build_res_block(256, 256, opt.padding_type, opt.use_instance_norm, 1))
...
--   seq5_ = nn.Sequential()
--   seq5_:add(nn.SpatialConvolution(256, 256, 1, 1, 1, 1))
--   seq5_:add(nn.ReLU(true))
...
--   seq4 = nn.Sequential()
--   seq4:add(build_res_block(128, 256, opt.padding_type, opt.use_instance_norm, 2))
--   seq4:add(nn.ReLU(true))
--   seq4:add(bch4)
--   seq4:add(nn.SpatialConvolution(512, 128, 3, 3, 1, 1, 1, 1))
--   seq4:add(nn.InstanceNormalization(128))
--   seq4:add(nn.ReLU(true))  
--   seq4:add(nn.SpatialFullConvolution(128, 128, 3, 3, 2, 2, 1, 1, 1, 1))  
